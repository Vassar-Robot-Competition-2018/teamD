...
void FollowBlock()
{
  ...
  if ((current - lastWheelUpdate) >= wheelInterval) { // time to update
    ...
    /*   if (followError > 15) {   // test for a good minimum error
         speedL = (int)constrain(baseSpeedL + wheelAdj, minSpeed, maxSpeed);
         speedR = (int)constrain(baseSpeedR - wheelAdj, minSpeed, maxSpeed);
       }
       else {
         speedL = (int)constrain(highSpeed + wheelAdj, minSpeed, maxSpeed);
         speedR = (int)constrain(highSpeed - wheelAdj, minSpeed, maxSpeed);
       }
    */
...
void TOF() {
  ...
  if ((current - lastTOFupdate) >= TOFinterval) { // time to update
    ...
    if ((measure.RangeStatus != 4) && ((measure.RangeMilliMeter > 10) && (measure.RangeMilliMeter < 1200))) {
