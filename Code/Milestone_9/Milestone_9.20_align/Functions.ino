...
void Align() {
  if (newQuad == 1) {
    MotorUpdate(0, baseSpeedR);
  }
  else if (newQuad == 2) {
    MotorUpdate(baseSpeedL, 0);
  }
}
  ...
  if (Home != 5) {
    ...
    if (blocks) { //if a color sig is detected by pixy cam
      for (j = 0; j < blocks; j++) { //find the largest signature
        if ((pixy.blocks[j].height > 20) && (pixy.blocks[j].height < 150)) {
          if ((pixy.blocks[j].width > 20) && (pixy.blocks[j].width < 150)) {
            maxJ = j;
